...
class Xhssjb extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data:[],
                ...
                if (value !== '0') {
                  return (
                    <div>{value}
                        ...
                        <a onClick={()=> {this.download(1,value,row)}}><Icon type="download" /></a>
                    </div>)
                }else{
                  return value;
                }
    ...
    fetch = ( params ={} ) => {
        ...
        ajaxUtil("urlencoded","xhssjb!getJsonList.action?tableName=xhssjb",text,this,(data,that) => {
    ...
    exportMes=(e)=>{
         ...
         +"&endDate="+endDate+"&tableName=xhssjb";
        //  if (e.key==='1') {
        ...
        //     text+="&flag=all&bizeScopes=";
            // this.help(text);
            window.location.href="/DDCMS/xhssjb!loadXLS.action?"+text;
        //  }
       }
    reflash=() => {
        this.fetch();
    ...
    // loadTreeNode =() =>{
    //     ajaxUtil("urlencoded","xhssjb!getJsonList.action?tableName=xhssjb","",this,(data,that) =>{
