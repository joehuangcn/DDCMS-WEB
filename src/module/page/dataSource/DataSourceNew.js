  ...
  getFields=() =>{
    ...
    return(
      ...
      <div className="fileChoice" style={{display: choiceWay==='F'? 'block' : 'none'}}>
       ...
       <Row>
       <Col span={12}>
       {this.handleRenderTab("input",inputFormItemLayout,"主机/IP","dhost",choiceWay==='D'?true:false,record.dhost,[],0,'')}
       </Col>
       <Col span={12}>
       ...
       <div className="ftpChoice" style={{display: choiceWay==='L'? 'block' : 'none'}}>
         ...
         <Row>
         <Col span={12}>
         {this.handleRenderTab("input",inputFormItemLayout,"主机/IP","lhost",choiceWay==='L'?true:false,record.lhost,[],0,'')}
         </Col>
         <Col span={12}>
...
class DataSourceNew extends Component{
  constructor(props){
    super(props);
    this.state={
        visible:false,
  ...
  handleOk=(e) =>{
    ...
    form.validateFields(( err, values) => {
      ...
      ajaxUtil("urlencoded","data-source-config!saveDataSourceConfig.action",text,this,(data,that) => {
        let status=data.success;
        let message= data.message;
        this.setState({
          visible: false,
