...
import DataSourceNewMs from './DataSourceNewMs';
import DataSourceNew from "./DataSourceNew";
const {RangePicker} = DatePicker;
const {Option} = Select;
const {Search} =Input;

class DataSourceMg extends Component {
  constructor(props){
    super(props);
    this.state={
      columns:[],
  ...
  setHead=() =>{
    const columns =[
        { title:'数据源名称', dataIndex:'dscname' , key:'dscname'},
        { title:'数据采集方式', dataIndex:'gatherway' , key:'gatherway',render:(text) =>(this.renderStatu(text))},
    ];
      ...
      render : (text, record) => {
        ...
        <span>
         ...
         <Popconfirm title="你确定要删除该记录?" okText="是" cancelText="否" disabled onConfirm={() => {
           ajaxUtil("urlencoded","data-source-config!delDSCbyID.action","id="+record.id,this,(data,that) => {
  ...
  fetch = ( params ={} ) => {
    ...
    ajaxUtil("urlencoded","data-source-config!getDataSourceConfigList.action",text,this,(data,that) => {
  ...
  render() {
    ...
    <DataSourceNew ref={(ref) => this.newbiz=ref }
