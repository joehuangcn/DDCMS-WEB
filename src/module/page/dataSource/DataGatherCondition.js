...
import DataGatherMes from "./DataGatherMes";
const {RangePicker} = DatePicker;
const {Option} = Select;
const {Search} =Input;
// 数据采集条件管理
class DataGatherCondition extends Component {
  constructor(props){
    super(props);
    this.state={
      columns:[],
  ...
  setHead=() =>{
    const columns =[
        ...
        { title:'数据采集方式', dataIndex:'gatherType' , key:'gatherType',render:(text) =>(this.renderStatu(text))},
    ];
      ...
      render : (text, record) => {
        ...
        <span>
         ...
         <Popconfirm title="你确定要删除该记录?" okText="是" cancelText="否" disabled onConfirm={() => {
           ajaxUtil("urlencoded","data-gather-conditions!delDGCbyID.action","id="+record.id,this,(data,that) => {
  ...
  fetch = ( params ={} ) => {
    ...
    ajaxUtil("urlencoded","data-gather-conditions!getDataSourceConfigList.action",text,this,(data,that) => {
  ...
  render() {
    ...
    <DataGatherMes ref={(ref) => this.newbiz=ref }
...
export default DataGatherCondition;
